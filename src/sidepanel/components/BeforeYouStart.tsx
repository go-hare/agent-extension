 ...
 *
 * When `embedded` (official EZ mcpPermissionOnly gate), outer already
 * centers with p-3 + max-w-md — skip the full-screen shell (xZ inside EZ).
 */
...
export interface BeforeYouStartProps {
  onContinue: () => void;
  /** Official EZ embeds this in max-w-md; skip full-screen chrome. */
  embedded?: boolean;
}

export function BeforeYouStart({ onContinue, embedded = false }: BeforeYouStartProps) {
  const t = useUi();

  const body = (
    <div className={embedded ? 'w-full' : 'max-w-[520px] w-full'}>
